...
class TestRunCatchUp:
    ...
    async def test_skips_too_recent_commitments(
        ...
        """Skips commitments that are too recent in catch-up phase."""
        mock_downloader = MagicMock()
        mock_downloader.is_cached.return_value = False

        # Commitment is too recent: block 9950, metagraph block 10000, cutoff 9900
        recent_commitment = MagicMock()
        recent_commitment.hotkey = "5RecentHotkey"
        recent_commitment.model_hash = "hash123"
        recent_commitment.block_number = 9950
        ...
        results = await scheduler.run_catch_up(failed_hotkeys={"5RecentHotkey"})

        # Should not retry - commitment is too recent
        assert results == {}
        mock_downloader.download_model.assert_not_called()

    @pytest.mark.asyncio
    async def test_retries_old_but_skips_recent_in_catch_up(
        self,
        scheduler_config: SchedulerConfig,
        mock_chain_client: MagicMock,
    ) -> None:
        """Retries old commitments but skips recent ones in the same catch-up."""
        from real_estate.models.downloader import ModelDownloadResult
        ...
        old_commitment = MagicMock()
        old_commitment.hotkey = "5OldHotkey"
        old_commitment.model_hash = "hash_old"
        old_commitment.block_number = 1000

        recent_commitment = MagicMock()
        recent_commitment.hotkey = "5RecentHotkey"
        recent_commitment.model_hash = "hash_new"
        recent_commitment.block_number = 9950

        mock_chain_client.get_all_commitments = AsyncMock(
            return_value=[old_commitment, recent_commitment]
        )
        mock_chain_client.get_metagraph = AsyncMock(
            return_value=MagicMock(block=10000)
        )
        ...
        scheduler._known_commitments = {
            "5OldHotkey": old_commitment,
            "5RecentHotkey": recent_commitment,
        }

        results = await scheduler.run_catch_up(
            failed_hotkeys={"5OldHotkey", "5RecentHotkey"}
        )

        # Only old commitment should be retried
        assert "5OldHotkey" in results
        assert "5RecentHotkey" not in results
        mock_downloader.download_model.assert_called_once()
